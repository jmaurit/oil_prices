...
One objection to this model may be that a fixed effect controlling for the general production profile of oil fields does not sufficiently address idiosyncratic differences between the fields. To address this I can add field random effects that takes into account uncertainty based on random variation between the fields. The smoothed function can then be written as $f(production\_time_{i,t})*Z$.  Where $Z~N(0, \sigma)$ and $\sigma$ is to be estimated.  
...
In the formulation with a two-dimensional smoothed function, it is not possible to estimate a random effects component for computational reasons. 
